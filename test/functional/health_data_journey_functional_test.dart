...
void main() {
  group("Health data journeys", () {
    ...
    test("health roster bulk save is visible after reload", () async {
      ...
      expect(
        await repository.getByClassDate(classId: 301, assessmentDate: "2026-06-10"),
        isEmpty,
      );
    ...
    test("history and latest return measures for student 401", () async {
      final repository = HealthAssessmentRepository(apiClient: client);
      ...
      expect(history.any((item) => item.heightCm > 0 && item.weightKg > 0), isTrue);
    });

    test("approved class transfer changes student enrollment and revert restores it", () async {
