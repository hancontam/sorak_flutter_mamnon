...
| SORAK-TEST-011 | High | Accepted scope | Parent Health/Growth endpoints do not exist | Product owner confirmed current backend does not support these APIs; live UI shows an explicit unavailable state and never mock data | Reopen only when backend accepts `backend_parent_read_api_contract.md` |
